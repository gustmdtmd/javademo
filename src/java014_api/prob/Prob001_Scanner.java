...
public class Prob001_Scanner {
	public static void main(String[] args) {
		...
		int userNum = 70;// 사용자가 입력할 값을 저장할 변수
		int count = 0;// 실행횟수를 저장할 변수
		// randomNum과 userNum이 일치할때 까지 반복하여 입력받아 평가하는 코드를 작성하세요.

		while (true) {
			System.out.print("1과 100까지 숫자를 입력하세요:");
			randomNum = key.nextInt();
			if (userNum > randomNum) {
				count++;
				System.out.println("1과 100사이의 값 중 입력한 값보다 큰값을 입력하세요.");
			} else if (userNum < randomNum) {
				count++;
				System.out.println("1과 100사이의 값 중 입력한 값보다 작은값을 입력하세요.");
			} else{
				System.out.println("사용자가 입력한 값과 컴퓨터가 발생시킨 임의의 값이 일치합니다.");
				System.out.println("실행 횟수는 " + count + "번 입니다.");
				break;
			}
